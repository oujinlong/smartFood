const lang = {
	common: {
		Notice: 'Notice',
		Cancel: 'Cancel',
		Yes: 'Yes',
		Name: 'Name',
		Phone: 'Phone',
		Remark: 'Remark',
		Time: 'Time',
		Detail: 'Detail',
		Contactus: 'Contact Us',
    Login: 'Login',
    Submit: 'Submit',
    Succeed: 'Succeed',
    Telephone: 'Telephone',
    Address: 'Address',
    AboutUs: 'About Us',
    OK: 'OK',
    Successful: 'Successful',
    Order: 'Order',
    OrderDetail: 'Order Detail',
    reservationOrderDetail: 'Order Detail',
    more: 'More',
    Home: 'Home'
	},
	errorMessage: {
    pleaseinputyourname:'please input your name',
    pleaseinputyourphonenum: 'please input your phone num',
    pleaseinputcurrentphonenum: 'please input current phone num'
	},
	paymentDialog: {
    Paymentoptions: 'Payment options',
    Youselected: 'You selected',
    Confirmpayment: 'Confirm payment'
	},
	search: 'Search',
	more_lang_change_finish: 'Language Changed ',
	location: 'Location',
	Categories: 'Categories',
	openHours: 'Open Hours',
	promotion: 'Promotion',
	Weekday: 'Weekday',
	Weekend: 'Weekend',
	RestaurantCategory: 'Store Category',
	FloorDirection: 'Floor,Direction',
	Nodata: 'No data',
	Reservation: 'Reservation',
	SelfTaking: 'Self-taking',
	DinedIn: 'Dined-In',
	OrderID: 'Order',
	ArrivalTime: 'Arrival time',
	Countofmeals: 'Count of meals',
	Detail: 'Detail',
	Tabletype: 'Table type',
  adCloseTip: 'Tap To Close',
  merchantCloseStatusClose: 'Merchant Closed',
  merchantCloseStatusOpen: 'In Business',
  merchantCloseTip: 'Merchant Closed',
  map: 'Map',
	reservation: {
    reservation: 'Reservation',
		Applyforrefund: 'Apply for refund',
		Delete: 'Delete',
		Cancel: 'Cancel',
		ReservingFilter: 'Reserving',
		ReservedFilter: 'Reserved',
		CancelledFilter: 'Cancelled',
		PendingrefundFilter: 'Pending refund',
		RefundsuccessfulFilter: 'Refund successful',
		RefundfailedFilter: 'Refund failed',
		Doyouneedtoapplyforarefund: 'Do you need to apply for a refund?',
		Refunded: 'Refunded',
		Tryagainlater: 'Try again later',
		Deletethereservation: 'Delete the reservation?',
		Deleted: 'Deleted',
		Cancelthereservation: 'Cancel the reservation?',
		Cancelled: 'Cancelled',
		Numberofpeople: 'Number of people',
		Reservationrequestisreceived: 'Reservation request is received',
		Confirmationinprogress: 'Confirmation in progress',
		Reservationsuccessfully: 'Reservation successfully',
		Reservationiscancelled: 'Reservation is cancelled',
		Returnapplication: 'Return application',
		Returnrequestreview: 'Return request review',
		Refundsuccessfully: 'Refund successfully',
		Refundfailed: 'Refund failed',
    Reservation: 'Reservation',
    Date: 'Date',
    Arrivaltime: 'Arrival time',
    Tabletype: 'Table type',
    Theminimumconsumption: 'The minimum consumption',
    Advancepayment: 'Advance payment',
    Theservicefee: 'The service fee',
    Tips:'Tips',
    Confirmtocancelthisorder: 'Confirm to cancel this order ?'
	},
	more: {
    Hellotourists: 'Hello, tourists',
    touristsText :'In order to give you a better shopping experience, we invite you to log in before making a purchase.',
    Wallet: 'Wallet',
    piece: 'piece',
    Coupon: 'Coupon',
    point: 'point',
    Score: 'Score',
    DeliveryAddress: 'Delivery Address',
    RechargeCenter: 'Recharge Center',
    CouponCollection: 'Coupon Collection',
	  MyCoupon: 'My Coupon',
    OnlineService: 'Online Service',
    HelpCenter: 'Help Center',
    ChangeLanguage: 'Change Language',
    Rechargeactivity: 'Recharge activity',
		currentBalance: 'Current Balance',
		rechargeAmount: 'Recharge Amount',
		giftAmount: 'Gift Amount',
    available: 'available',
    upto: 'up to',
    isavailable: 'is available',
    Nothreshold: 'No threshold',
    RequireCoupon:'Require Coupon',
    Instructions: 'Instructions',
    RequireVoucher: 'Require Voucher',
    Nocoupons: 'No coupons'
  },
	store: {
	  SearchStore: 'Search Store',
    KeySearch: 'Key Search',
    TableReservation: 'Table Reservation',
    ScanQRcode: 'Scan QRcode',
    OrderMenu: 'Order Menu',
    Payment:'Payment',
    MerchantsInfo: 'Merchants Info',
    Announcement: 'Announcement',
    Environment: 'Environment',
    Awards: 'Awards'
	},
	orderMenu: {
    OrderMenu: 'Order Menu',
    Reviews: 'Reviews',
    Detail: 'Detail',
    Month: 'Month',
    Next: 'Next',
    Cart: 'Cart',
    Clear: 'Clear',
    TotalScore: 'Total Score',
    Rate:'Rate',
    Score: 'Score',
    Comment: 'Comment(s)',
    FoodDetail: 'Food Detail',
    monthlySale: 'Monthly',
    stock: 'Stock',
    goodDesc: 'Description',
    tip: 'Warm Tips: Pictures are for reference only, please refer to the real object; Peak hours and bad weather, please place an order in advance.',
    Coupons: 'Coupons',
    Onlinepaymentdiscount: 'Online payment discount',
    Newuserdiscount: 'New user discount',
    Taxes: 'Taxes',
    Total: 'Total',
    Remark: 'Remark',
    Discount: 'Discount',
    PayNow: 'Pay Now',
    Selftakingtime: 'Self-taking time',
    Spec: 'Specifications'
	},
  selfTaking: {
    etc: '.etc',
    Total: 'Total',
    TotalAmount: 'Total',
    Remind: 'Remind',
    Confirm: 'Confirm',
    Comment: 'Comment',
    Anotherorder: 'Another order',
    ConfirmReceipt: 'Confirm Receipt',
    Type: 'Type',
    OrderID: 'Order ID',
    OrderTime: 'Order Time',
    Discount: 'Discount',
    Tax: 'Tax',
    Contract: 'Contract',
    Actualpayment: 'Actual payment',
    Confirmtheorder: 'Confirm the order?',
    Payment: 'Payment',
    TableName: 'Table Name',
    TableNameType: 'Table Name Type',
    PendingPaymentFilter: 'Pending payment',
    OrderPendingFilter: 'Order pending',
    InDeliveryFilter: 'In delivery',
    FinishedFilter: 'Finished',
    CancelledFilter: 'Cancelled',
    PaddingRefundFilter: 'Padding refund',
    RefundSuccessfulFilter: 'Refund successful',
    RefundFailedFilter: 'Refund failed',
    title1: 'Wait for payment',
    title2: 'Waiting for order',
    title3: 'Merchant production',
    title4: 'Order Finished',
    title5: 'Order cancelled',
    title7: 'Waiting for review',
    title8: 'Refund successful',
    title9: 'Refund failed',
    tip1: 'If the payment is overdue, the order will be automatically taken',
    tip2: 'Merchant pending order',
    tip3: 'Being produced, please be patient',
    tip4: 'Product has been picked up',
    tip5: 'You have cancelled your order, welcome to visit next time',
    tip7: 'You have applied for a refund, welcome to visit next time',
    tip8: 'Your refund has been successful, welcome to visit next time',
    tip9: 'Your refund request has not been approved',
    titleDineIn1: 'Successful Dined-In',
    titleDineIn2: 'Order Finished',
    tipDineIn1: 'The merchant is preparing meals, please be patient',
    tipDineIn2: 'Thank you for your support, welcome to visit us next time',
    tipDineIn3: 'You have cancelled your order, welcome to visit next time'
  },
  comment: {
    Score: 'Score',
    start: 'start(s)',
    Commentget: 'Comment get',
    point: 'point(s)',
    Submit: 'Submit',
    Giveusyouradvice: 'Give us your advice'
  }
}
export default lang
